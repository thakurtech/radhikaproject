...
} from './lib/models'; // relative path instead of alias
...
async function runSeed() {
  try {
    ...
    if (!uri) throw new Error('No MONGODB_URI found in process.env');
    const dbUri = uri.replace('.env.local value or whatever', '').trim();
    
    console.log('Connecting to MongoDB...', dbUri.substring(0, 20) + '...');
    await mongoose.connect(dbUri);
    console.log('Connected.');
    
    // Hardcode hash since bcrypt is problematic without next runtime sometimes, using precomputed "$2b$10$wE9... "
    // Actually, bcryptjs is fine in node!
