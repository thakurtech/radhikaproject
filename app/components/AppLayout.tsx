...
import {
  ...
  Sparkles, LogOut, HelpCircle, School
} from 'lucide-react';
  ...
  return (
    <header className="topbar">
      ...
      <div className="topbar-actions">
        ...
        <div className="user-avatar avatar-md" style={{ background: 'linear-gradient(135deg, var(--primary), var(--accent-violet))', width: 34, height: 34, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', flexShrink: 0 }}>
          SA
        </div>
      </div>
    </header>
  ...
  return (
    <>
          ...
          <div className="ai-chat-messages">
            {messages.map((msg) => (
              <div key={msg.id} className={`ai-message ${msg.role}`}>
                {msg.role === 'bot' && (
                  <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary), var(--accent-violet))', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <button className="ai-fab" onClick={() => setOpen(!open)} aria-label="AI Assistant">
        ...
        {open ? <span style={{ fontSize: '1.1rem', fontWeight: 700, zIndex: 1 }}>✕</span> : <Sparkles size={22} style={{ zIndex: 1, position: 'relative' }} />}
      </button>
    </>
  );
}
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Don't show app layout on landing / auth pages
